...
const Game = () => {
  ...
  const moveToNextArea = () => {
    ...
    axios2
      .post("http://10.126.77.213:5000/predict", data)
      .then((res) => {
  ...
  useEffect(() => {
    if (wordChars.length === chars.length) {
      const isWordCorrect = chars.every(
        (char, index) => char === wordChars[index]
      );
      if (isWordCorrect) {
  ...
  return (
    ...
    >
        ...
        {currentWordIndex < words.length - 1 ? (
          <div>
            ...
            >
              ...
              >
                {`Task number ${currentWordIndex + 1}: ${
            ...
            {currentArea === "A" && (
              <div className=" mt-6 m">
                ...
                >
                  <div
                    className="text-5xl font-bold text-white border-2 flex justify-center items-center"
                    draggable
                    onDragStart={(e) => handleOnDrag(e, `${currentWord[2]}`)}
                  >
                    {" "}
                  ...
                  <div
                    className="text-5xl font-bold text-white border-2 flex justify-center items-center"
                    draggable
                    onDragStart={(e) => handleOnDrag(e, `${currentWord[0]}`)}
                  >
                  ...
                  <div
                    className="text-5xl font-bold text-white border-2 flex justify-center items-center"
                    draggable
                    onDragStart={(e) => handleOnDrag(e, `${currentWord[1]}`)}
                  >
          ...
          >
            <div className="p-6 rounded-lg  text-center">
              <p className="text-green-600 font-bold text-lg">
                Congratulations, <b className="text-red-600">{userName}</b>!
              </p>
              <p className="text-green-800">
                You have finished your first chapter.
              </p>
              <button
                className="mt-4 bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 focus:outline-none focus:shadow-outline"
                onClick={moveToNextArea}
              >
      ...
      {currentWordIndex < words.length - 1 && (
        <div>
          <div
            className="flex flex-col gap-6 items-end justify-center mx-7"
            style={{ height: "90vh", position: "absolute", top: 0, right: 0 }}
          >
            <img
              width="60"
              src={profil1}
              alt=""
              onClick={() =>
                setActivePanel(activePanel === "status" ? "" : "status")
              }
            />
            <img
              width="60"
